<?php
// $errorMessage = __( 'Invalid request. Maybe You\'re on our tracking for illigal interactions.', 'fwp-divi-child-seojaws' );
$errorHappens = false;$myCourses = get_query_var( 'mycourses' );
if( $myCourses != '' && get_query_var( 'download' ) != '' && get_query_var( 'course' ) != '' && is_user_logged_in() ) {
  $course = get_post_meta( get_query_var( 'course' ), '_fwp_course', true );
  if( $course && ! empty( $course ) ) {
    // $fwpCls = FWP_COURSES_MAIN_CLASS;
    $instead = FWP_COURSES_REPLACE_URL_INSTEAD;$replace = FWP_COURSES_REPLACE_URL_REPLACE;
    $filePath = get_query_var( 'download' );

    $downloadFileArgs = explode( FWP_COURSES_REPLACE_URL_SLASH, $filePath );$downloadFilePath = $downloadFileArgs;
    if( isset( $downloadFilePath[1] ) ) {unset( $downloadFilePath[0] );}
    $filePath = implode( '/', $downloadFilePath );

    $filePath = str_replace( $replace, $instead, urldecode( $filePath ) );
    $filePath = ( substr( $filePath, -1 ) == '/' ) ? substr( $filePath, 0, -1 ) : $filePath;

    $root = FWP_COURSES_LESSON_ROOT; // ABSPATH . 'lessons'
    $path = $root . '/' . $filePath;
    $mainFile = $path;

    if( file_exists( $mainFile ) || is_dir( $mainFile ) ) {
      global $wp;$user_id = get_current_user_id();
      $downloads = get_post_meta( get_query_var( 'course' ), '_fwp_course_downloads', true );$downloads = is_array( $downloads ) ? $downloads : [];
      // delete_post_meta( get_query_var( 'course' ), '_fwp_course_downloads' );

      if( $myCourses == 'download' ) {
        if( isset( $downloads[ 'u-' . $user_id ] ) && $course[ 'status' ] === true && ( $course[ 'url' ] == home_url( $wp->request . '/' ) || $course[ 'link' ] == get_query_var( 'download' ) ) && ( FWP_COURSES_MAX_DOWNLOADS > $downloads[ 'u-' . $user_id ] || FWP_COURSES_MAX_DOWNLOADS == 0 ) ) {
          if( ! class_exists( 'FWP_HZip' ) ) {include_once( get_stylesheet_directory() . '/inc/class-zip.php' );}

          $mainZipFile = $mainFile . '.zip';
          
          foreach( FWP_COURSES_ALLOWED_EXTENSIONS as $extension ) {
            if( file_exists( $mainZipFile . '.' . $extension ) && ! is_dir( $mainZipFile . '.' . $extension ) ) {
              $mainFile = $mainZipFile . '.' . $extension;
            }
          }
          // FWP_HZip::zipDir( $mainFile, $mainZipFile );$mainFile = $mainZipFile;

          if( file_exists( $mainZipFile ) && ! is_dir( $mainZipFile ) ) {
            $downloads[ 'u-' . $user_id ] = ( $downloads[ 'u-' . $user_id ] + 1 );
            update_post_meta( get_query_var( 'course' ), '_fwp_course_downloads', $downloads );

            // print_r( [ $mainZipFile, filesize( $mainZipFile ) ] );
            
            $mainFileSize = filesize( $mainZipFile );
            if( $mainFileSize <= 10 ) { // 10KiB.
              header( 'Content-Description: File Transfer' );
              header("Content-Type: application/force-download");
              header("Content-Type: application/octet-stream");
              header("Content-Type: application/download");
              header( 'Content-Type: application/zip' );
              header( 'Content-Disposition: attachment; filename="'.basename( $mainZipFile ).'"' );
              header('Content-Transfer-Encoding: binary');
              header( 'Expires: 0' );
              header( 'Cache-Control: must-revalidate, post-check=0, pre-check=0' );
              header( 'Pragma: public' );
              header( 'Content-Length: ' . $mainFileSize );
              // header("X-Sendfile: " . $mainZipFile);
              readfile( $mainZipFile );

              // $context = stream_context_create();
              // set_time_limit(0);
              // ob_clean();ob_end_flush();
              // $handle = fopen( $mainZipFile, 'rb', FALSE, $context );
              // while( ! feof( $handle ) ) {
              //   // echo stream_get_contents($file, 2014);
              //   echo fread($handle, 1000);ob_flush();flush();
              // }
              // fclose( $handle );
            } else {
              // str_replace( [ ABSPATH ], [ '' ], $root )
              wp_redirect( site_url( str_replace( [ ABSPATH, '\/' ], [ '/', '/' ], $mainZipFile  ) ) );
            }
          } else {
            $errorHappens = __( 'File not found on courses directory. Please contact to your Administartive.', 'fwp-divi-child-seojaws' );
          }
        } else {
          $errorHappens = __( 'Permission denaid for your download. Maybe you\'ve exceed your limit.', 'fwp-divi-child-seojaws' );
        }
      } else if( $myCourses == 'watch' ) {
        if( true ) {
          $sourceVideo = ABSPATH . 'lessons/' . $filePath;
          if( file_exists( $sourceVideo ) && ! is_dir( $sourceVideo) ) {
            $destinationVideo = FWP_COURSES_FORMATED_POSITION . pathinfo( $filePath, PATHINFO_DIRNAME ) . '/' . pathinfo( $filePath, PATHINFO_FILENAME ) . '-' . $downloadFileArgs[0] . '.' . pathinfo( $filePath, PATHINFO_EXTENSION );

            if( file_exists( $destinationVideo ) && ! is_dir( $destinationVideo ) ) {
              if( filesize( $destinationVideo ) > 0 ) {wp_redirect( str_replace( [ ABSPATH ], [ site_url( '/' ) ], $destinationVideo ) );} else {unlink( $destinationVideo );}
            }
            else {
              
              $videoArgs = [
                'src' => $sourceVideo, 'dest' => $destinationVideo, 'bitrate' => $downloadFileArgs[0] . 'p'
              ];
              $videoDestDir =  pathinfo( $videoArgs[ 'dest' ], PATHINFO_DIRNAME );
              is_dir( $videoDestDir ) || wp_mkdir_p( $videoDestDir ); // chmod( $cache_folder, 0777 );
    
              $videoResults = apply_filters( 'fwp/courses/convert/video', $videoArgs );
              
              if( isset( $videoResults[ 'success' ] ) && $videoResults[ 'success' ] ) {
                wp_redirect( str_replace( [ ABSPATH ], [ site_url( '/' ) ], $destinationVideo ) );
              } else {
                if( FWP_COURSES_ALLOW_SOURCE_ON_FAILCONVERT ) {
                  wp_redirect( str_replace( [ ABSPATH ], [ site_url( '/' ) ], $sourceVideo ) );
                } else {
                  if( true ) {
                    $errorHappens = __( 'Failed to convert files fro your tergatted formate. Sorry to say there is also no premission to give here source file.', 'fwp-divi-child-seojaws' );
                  } else {
                    wp_die(
                      print_r( [
                        $videoArgs,
                        $videoResults,
                        file_exists( $destinationVideo ) ? filesize( $destinationVideo ) : 'Not found'
                        // // get_query_var( 'mycourses' ),
                        // $filePath,
                        // $downloadFileArgs,
                        // get_query_var( 'course' )
                      ] ),
                      'Video Generating Fialed'
                    );
                  }
                }
              }
            ...
            $errorHappens = __( 'Targeted file not found from source directory.', 'fwp-divi-child-seojaws' );
          }
        } else {
          // header("video/x-flv");
          // $src = '/home/futurewo/lessons/View_From_A_Blue_Moon_Trailer-HD/View_From_A_Blue_Moon_Trailer-1080p.mp4';
          // $dest = '/home/futurewo/lessons/View_From_A_Blue_Moon_Trailer-HD/View_From_A_Blue_Moon_Trailer-480p.mp4';
          // passthru('ffmpeg -i ' . $src . ' -ab 96k -b 700k -ar 44100 -s 640x480 -acodec mp3 ' . $dest);
          // include_once( get_stylesheet_directory() . '/ffmpeg/index.php' );
          wp_head();
          ?>
            <video controls crossorigin playsinline data-a-poster="https://cdn.plyr.io/static/demo/View_From_A_Blue_Moon_Trailer-HD.jpg" id="player">
      ...
      } else if( $myCourses == 'subtitle' ) {
        $sourceSubTitle = ABSPATH . 'lessons/' . $filePath;
        if( file_exists( $sourceSubTitle ) && ! is_dir( $sourceSubTitle) ) {
          // wp_die( $sourceSubTitle, 'Hellow' );
          $args = [
            'src' => $sourceSubTitle,
            'dest' => false
          ];
          if( FWP_COURSES_ALLOW_SUBTITLE_CONVERT ) {
            $vtt = apply_filters( 'fwp/courses/convert/srt2vtt', $args );
            // https://stackoverflow.com/questions/16594185/php-output-to-file-for-download-without-creating-file-on-the-server
            header("Content-type: text/vtt");
            echo $vtt;
          } else {
            header("Content-type: text/srt");
            echo file_get_contents( $args[ 'src' ] ); 
          }

        } else {
          $errorHappens = 'Not found';
        }
      } else if( $myCourses == 'attachments' ) {
        $attachmentSrc = ABSPATH . 'lessons/' . $filePath;
        if( file_exists( $attachmentSrc ) && ! is_dir( $attachmentSrc ) ) {
          wp_redirect( str_replace( [ ABSPATH ], [ site_url( '/' ) ], $attachmentSrc ) );
        } else {
          $errorHappens = __( 'File not found.', 'fwp-divi-child-seojaws' );
        }
      } else {
        $errorHappens = __( 'Action not found. Maybe you\'ve changed something with your link or you\'ve viewed an unknown URL.', 'fwp-divi-child-seojaws' );
      }
      
      // print_r( $fwpCls->getRFsize( 'https://demo.futurewordpress.com/seojaws/wp-content/uploads/2022/01/0vedrqxxrqo-240x300.jpg' ) );
      
    } else {
      $errorHappens = __( 'Derectory not found.', 'fwp-divi-child-seojaws' );
    }
	} else {
    // echo esc_html( $errorMessage );
    $errorHappens = __( 'Sorry! Weve detected your link as a currupted url.', 'fwp-divi-child-seojaws' );
  }
} else {
  // echo esc_html( $errorMessage );
  $errorHappens = __( 'Maybe you\'re not logged it or your URL is not valid.', 'fwp-divi-child-seojaws' );
}

if( $errorHappens ) {
  // http_response_code( 404 );
  // status_header( 404, 'Course not found' );
  add_filter( 'pre_get_document_title', function( $title ) {return __( '404:( Targeted Course not found || Please contact with support or site admin.', 'fwp-divi-child-seojaws' );}, 10, 1 );
  get_header();
  ...
  <style>
    #handboy{ animation: swing ease-in-out 1.3s infinite alternate; transform-origin: 98% 98%; transform-box: fill-box;} #girllight{ animation: swing ease-in-out 1.3s infinite alternate; transform-origin: 0% 97%; transform-box: fill-box;} #hairgirl{ animation: swinghair ease-in-out 1.3s infinite alternate; transform-origin: 60% 0%; transform-box: fill-box;} #zero{ transform-origin:bottom; transform-box:fill-box;} @keyframes swing{ 0%{ transform: rotate(10deg);} 100%{ transform: rotate(-10deg);}} @keyframes swinghair{ 0%{ transform: rotate(6deg);} 100%{ transform: rotate(-6deg);}}body.fwp-body, .fwp-body .et_pb_section.et_pb_section_1_tb_header.et_section_regular.et_pb_section--with-menu, .fwp-body .et_pb_menu_0_tb_header.et_pb_menu { background-color:#141019; background: radial-gradient(at 50% -20%, #908392, #0d060e) fixed;}.fwp-body .et_pb_menu_0_tb_header .et_pb_menu_inner_container>.et_pb_menu__logo-wrap .et_pb_menu__logo img {filter: invert(1);mix-blend-mode: multiply;mix-blend-mode: color;}
  </style>
  <?php
  get_footer();
}
